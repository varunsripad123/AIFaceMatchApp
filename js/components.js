    ...
    return `
        <div class="photo-gallery">
            ${photos.map((photo, index) => `
                <div class="photo-card" onclick="${onPhotoClick ? `openLightbox(${index}, '${photo.id}')` : ''}" tabindex="0" onkeydown="if(['Enter',' '].includes(event.key)){this.click();event.preventDefault();}">
...
function renderLightbox() {
    ...
    lightbox.innerHTML = `
        <button class="lightbox-close" onclick="closeLightbox()" aria-label="Close gallery">✕</button>
...
function renderEventCard(event, onClick) {
    ...
    return `
        <div class="event-card" onclick="${onClick}" tabindex="0" onkeydown="if(['Enter',' '].includes(event.key)){this.click();event.preventDefault();}">
